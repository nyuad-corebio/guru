...
from tenx_dags.ssh_helpers import execute_ssh_command, initialize_ssh, execute_ssh_command_return_stdout_stderr
from tenx_dags.slurm_helpers import get_job_status, poll_slurm_job, parse_slurm_submission
...
def generate_demultiplex_command(kwargs):
    ...
    workflow = dag_run.conf['workflow']
    
    if workflow == 'cellranger':
        demultiplex_command = """cd {scratch_dir} && \\
        cellranger mkfastq -R {scratch_dir} \\
        --csv=10xsimple.csv --id={jira_ticket} \\
        --output-dir {jira_ticket} --jobmode=local \\
        --localcores=28 --localmem=400""".format(
            scratch_dir=scratch_dir,
            jira_ticket=jira_ticket,
            workflow=workflow
        )
    elif workflow == 'supernova':
        demultiplex_command = """cd {scratch_dir} && \\
        supernova mkfastq -R {scratch_dir} \\
        --csv=10xsimple.csv --id={jira_ticket} \\
        --output-dir {jira_ticket} --jobmode=local \\
        --localcores=28 --localmem=400""".format(
            scratch_dir=scratch_dir,
    ...
    elif workflow == 'longranger':
        demultiplex_command = """cd {scratch_dir} && \\
        longranger mkfastq -R {scratch_dir} \\
        --csv=10xsimple.csv --id={jira_ticket} \\
        -- qc \\
        --output-dir {jira_ticket} --jobmode=local \\
        --localcores=28 --localmem=400""".format(
    ...
    elif workflow == 'cellranger-atac':
        demultiplex_command = """cd {scratch_dir} && \\
        cellranger-atac mkfastq -R {scratch_dir} \\
        --csv=10xsimple.csv --id={jira_ticket} \\
        --output-dir {jira_ticket} --jobmode=local \\
        --localcores=28 --localmem=400""".format(
            scratch_dir=scratch_dir,
